...
test('download keeps a visible fallback link when automatic download is blocked',()=>{
  ...
  const events=[];
  let fallbackLink;
  const actions={insertAdjacentElement(position,element){events.push(['insert-after-actions',position,element])}};
  ...
  const context={
    ...
    document:{body:{querySelector(){return actions},insertBefore(){events.push(['insert-first'])}},createElement:makeElement},
    $(){return null},
  ...
  assert.deepEqual(events.map(x=>x[0]),['create','insert-after-actions','scroll','click','scheduled']);
