...
-- This narrow internal writer is callable by signed-in users but is not exposed
-- through the Data API. It only accepts an existing record owned by the patient.
create or replace function private.log_record_activity(
  p_action_type text,
  p_record_id uuid,
  p_patient_id uuid,
  p_metadata jsonb,
...
begin
  if auth.uid() is null or not private.is_active_user() then raise exception 'not authorized'; end if;
  if p_action_type not in ('record_created','record_updated') then raise exception 'unsupported activity type'; end if;
  if not exists(
    select 1 from public.records r
    where r.id=p_record_id and r.patient_id=p_patient_id
  ) then raise exception 'record not found'; end if;
  return public.log_activity(p_action_type,'record',p_record_id::text,p_patient_id,p_metadata,p_change_summary);
end$$;
revoke all on function private.log_record_activity(text,uuid,uuid,jsonb,text) from public,anon;
grant execute on function private.log_record_activity(text,uuid,uuid,jsonb,text) to authenticated;
...
begin
  ...
  perform private.log_record_activity(case when is_new then 'record_created' else 'record_updated' end,rid,p_patient_id,
